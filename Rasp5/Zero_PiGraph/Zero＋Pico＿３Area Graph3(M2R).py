...
def plot_data(start_time=None, y_scales=None):
    ...
    while True:
        ...
        bed_data = pd.read_csv("/home/koguretaichi/Documents/Flask/BedRoomEnv.csv")
        out_data = pd.read_csv("/home/koguretaichi/Documents/Flask/OutsideEnv.csv")
        lroom_data = pd.read_csv("/home/koguretaichi/Documents/Pi5_EnvData.csv")
        pico_data = pd.read_csv("/home/koguretaichi/Documents/Flask/PicodataX.csv")
        lr_data = pd.read_csv("/home/koguretaichi/Documents/Flask/LR_env.csv")
        ...
        pico_data['current_time'] = pd.to_datetime(pico_data['current_time'])
        pico_data.set_index('current_time', inplace=True)

        lr_data['current_time'] = pd.to_datetime(lr_data['current_time'])
        lr_data.set_index('current_time', inplace=True)
        ...
        plt.plot(lr_data.index, lr_data['CO2'], label='CO2_LR', color='k')
        ...
        plt.plot(pico_data.index, pico_data['Tempereture'], label='Outside Temperature-2', color='blue',
                 linestyle='solid')
        plt.plot(lr_data.index, lr_data['Temperature_DS18B20'], label='temp_C_Sensor2', color='brown',
                 linestyle='dotted')
        ...
        plt.plot(lr_data.index, lr_data['Humidity_DHT11'], label='Humid_LR', color='cyan', linestyle='dashdot')
        plt.ylabel('Humidity')
        plt.grid(True)
        ...
        plt.plot(out_data.index, out_data['GasResistance-outside'], label='Outside GasResistance', color='m',
                 linestyle='dashed')
        plt.plot(lr_data.index, lr_data['AnalogValue'], label='Gas_LR', color='gray', linestyle='solid')
        ...
        plt.draw()
        plt.pause(30)  # Pause for 30 seconds
