    ...
    def load_all_datasets(self):
        ...
        for key, file_path in self.data_files.items():
            try:
                ...
                print(f"{key}のデータ読み込み中にエラーが発生しました: {e}")
...
# Set style for plots
    ...
    def _create_trend_plot(self, data: Dict[str, pd.DataFrame], metric: str, 
                          subplot_pos: int, title: str, ylabel: str) -> None:
        plt.subplot(2, 2, subplot_pos)
        for location, df in data.items():
            column = self.SENSOR_MAPPINGS[metric].get(location)
        ...
        plt.ylabel(ylabel)
        plt.xticks(PlotConstants.HOUR_TICKS)
        plt.legend()
        plt.grid(True)
    ...
    def analyze_correlations(self):
        ...
        # ここに相関分析のコードを実装
        pass
        """環境データの日次トレンドを分析・可視化する"""
        plt.figure(figsize=PlotConstants.FIGURE_SIZE)

        # 温度トレンド
        self._create_trend_plot(
            self.datasets, 'temperature', 1,
            'Average Temperature by Hour of Day', 'Temperature (°C)'
        )

        # 湿度トレンド
        self._create_trend_plot(
            self.datasets, 'humidity', 2,
            'Average Humidity by Hour of Day', 'Humidity (%)'
        )

        # CO2トレンド
        self._create_trend_plot(
            self.datasets, 'co2', 3,
            'Average CO2 by Hour of Day', 'CO2 (ppm)'
        )
    ...
    # Plot 4: Absolute Humidity by hour of day
        ...
        for key, data in self.datasets.items():
            stats = data.describe()
            stats.to_csv(f"{self.output_dir}/{key}_basic_stats.csv")
            if 'Absolute_Humidity' in data.columns:
                hourly_abs_hum = data.groupby('hour')['Absolute_Humidity'].mean()
                plt.plot(hourly_abs_hum.index, hourly_abs_hum.values, 
                        label=location.replace('_', ' ').title())
        ...
        plt.tight_layout()
        plt.savefig(f'{self.output_dir}/daily_trends.png', dpi=PlotConstants.DPI)
        plt.close()
...
def analyze_environmental_data():
    # プロジェクトのルートパスを設定
    base_path = r"G:\Study2024-2025\TempAnalysis\Analysis1"
    
    # データファイルのパスを定義
    data_files = {
        'bedroom': f"{base_path}/BedRoomEnv.csv",
        'living_room': f"{base_path}/LR_env.csv",
        'outside': f"{base_path}/OutsideEnv.csv",
        'desk': f"{base_path}/Env_data_BME680.csv"
    }
    ...
    # アナライザーのインスタンスを作成
    analyzer = EnvironmentalDataAnalyzer(data_files, output_dir)
